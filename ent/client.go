...
func (c *Client) Intercept(interceptors ...Interceptor) {
	c.GithubUser.Intercept(interceptors...)
}

// Mutate implements the ent.Mutator interface.
func (c *Client) Mutate(ctx context.Context, m Mutation) (Value, error) {
	switch m := m.(type) {
	case *GithubUserMutation:
		return c.GithubUser.mutate(ctx, m)
	default:
...
// A call to `Intercept(f, g, h)` equals to `githubuser.Intercept(f(g(h())))`.
func (c *GithubUserClient) Intercept(interceptors ...Interceptor) {
	c.inters.GithubUser = append(c.inters.GithubUser, interceptors...)
}

// Create returns a builder for creating a GithubUser entity.
...
func (c *GithubUserClient) MapCreateBulk(slice any, setFunc func(*GithubUserCreate, int)) *GithubUserCreateBulk {
	rv := reflect.ValueOf(slice)
	if rv.Kind() != reflect.Slice {
		return &GithubUserCreateBulk{err: fmt.Errorf("calling to GithubUserClient.MapCreateBulk with wrong type %T, need slice", slice)}
	}
	builders := make([]*GithubUserCreate, rv.Len())
	for i := 0; i < rv.Len(); i++ {
		builders[i] = c.Create()
...
func (c *GithubUserClient) UpdateOne(gu *GithubUser) *GithubUserUpdateOne {
	mutation := newGithubUserMutation(c.config, OpUpdateOne, withGithubUser(gu))
	return &GithubUserUpdateOne{config: c.config, hooks: c.Hooks(), mutation: mutation}
}

// UpdateOneID returns an update builder for the given id.
func (c *GithubUserClient) UpdateOneID(id uuid.UUID) *GithubUserUpdateOne {
...
func (c *GithubUserClient) DeleteOne(gu *GithubUser) *GithubUserDeleteOne {
	return c.DeleteOneID(gu.ID)
}

// DeleteOneID returns a builder for deleting the given entity by its id.
func (c *GithubUserClient) DeleteOneID(id uuid.UUID) *GithubUserDeleteOne {
...
func (c *GithubUserClient) Query() *GithubUserQuery {
	return &GithubUserQuery{
		config: c.config,
		ctx:    &QueryContext{Type: TypeGithubUser},
		inters: c.Interceptors(),
	}
}

// Get returns a GithubUser entity by its id.
func (c *GithubUserClient) Get(ctx context.Context, id uuid.UUID) (*GithubUser, error) {
	return c.Query().Where(githubuser.ID(id)).Only(ctx)
}

// GetX is like Get, but panics if an error occurs.
func (c *GithubUserClient) GetX(ctx context.Context, id uuid.UUID) *GithubUser {
...
func (c *GithubUserClient) Interceptors() []Interceptor {
	return c.inters.GithubUser
}

func (c *GithubUserClient) mutate(ctx context.Context, m *GithubUserMutation) (Value, error) {
	switch m.Op() {
	case OpCreate:
		return (&GithubUserCreate{config: c.config, hooks: c.Hooks(), mutation: m}).Save(ctx)
	case OpUpdate:
		return (&GithubUserUpdate{config: c.config, hooks: c.Hooks(), mutation: m}).Save(ctx)
	case OpUpdateOne:
		return (&GithubUserUpdateOne{config: c.config, hooks: c.Hooks(), mutation: m}).Save(ctx)
	case OpDelete, OpDeleteOne:
		return (&GithubUserDelete{config: c.config, hooks: c.Hooks(), mutation: m}).Exec(ctx)
	default:
		return nil, fmt.Errorf("ent: unknown GithubUser mutation op: %q", m.Op())
	}
}
